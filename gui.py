...
class showSettingsWindow():
    def show(self,name,port,onlineMode,CBE,PVP,D,GM):
        ...
        ToolTip(name_Label,"Описание сервера, отображаемое при подключении в списке серверов. Поддерживает форматирование текста.")
        self.name_Entry = ttk.Entry(self.root,style="TEntry")
        self.name_Entry.place(x=120,y=0,width=190,height=20)
        ToolTip(self.name_Entry,"Описание сервера, отображаемое при подключении в списке серверов. Поддерживает форматирование текста.")

        port_Label = Label(self.root,text="Порт сервера:",font="arial 10")
        port_Label.place(x=0,y=40)
        ToolTip(port_Label,"Данный параметр определяет значение порта в протоколах TCP и UDP, который будет использовать сервер игры")
        ...
        ToolTip(onlineMode_Check, "Этот параметр позволяет включить/выключить проверку подлинности премиум-аккаунтов пользователей, которые подключаются к данному серверу.")
        ...
        ToolTip(CBE_Check, "Позволяет использовать командный блок. Эта опция не генерируется при первой загрузке, а появляется при первой попытке использования командного блока.")
        ...
        self.D_Combo = ttk.Combobox(self.root,values = ["Мирная","Лёгкая","Нормальная","Сложная"],height=4,state="readonly")
        ...
        self.GM_Combo = ttk.Combobox(self.root,values = ["Выживание","Креатив","Приключение","Наблюдатель"],height=4,state="readonly")
...
class showServerInfoWindow():
    def show(self,name,port,onlineMode,CBE,PVP,D,GM):
        ...
        nextButton = ttk.Button(self.root,text="собрать!",style="TButton",command=self.nextAction) #Создание кнопки со свойствами
        nextButton.place(x=485,y=370,width=100,height=25) #Размещение кнопки по кординатам и установка площади
